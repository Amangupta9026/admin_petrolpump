...
class HomeNotifier extends AutoDisposeAsyncNotifier<HomeMode> {
  ...
  Future onExcelSubmit() async {
    ...
    for (int i = 0; i < qs.docs.length; i++) {
      ...
      cell5.value = qs.docs[i].data()['selected_category'];

      var cell6 = sheetObject.cell(CellIndex.indexByString('F${i + 1}'));
      cell6.value = qs.docs[i].data()['selected_company'];

      var cell7 = sheetObject.cell(CellIndex.indexByString('G${i + 1}'));
      cell7.value = qs.docs[i].data()['application_number'];

      var cell8 = sheetObject.cell(CellIndex.indexByString('H${i + 1}'));
      cell8.value = trimDateTime(datetrim);
    ...
    List<int>? fileBytes = excel.save(fileName: 'PetrolExportData.xlsx');
  ...
  // Delete Doc
  Future<void> deleteDocument(BuildContext context, String email) async {
    EasyLoading.show(status: 'loading...');

    try {
      var getDocumentID = await FirebaseFirestore.instance
          .collection('queryForm')
          .where('email', isEqualTo: email)
          .get();
      var docId = getDocumentID.docs.first.id;
